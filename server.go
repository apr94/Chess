				...
				func playHandler(w http.ResponseWriter, r *http.Request, arg string){

					htmlTemplate := "PlayGame.html"
					pagePath := filepath.Join(tmplPath, htmlTemplate)
					...
					err = db.QueryRow("select hash_value from Games where game_id = ?", id).Scan(&password)
					err = db.QueryRow("select game_name from Games where game_id = ?", id).Scan(&GameName)

					if strings.EqualFold(r.FormValue("Password"), password) == true{
						...
						var templates = template.Must(template.ParseFiles(pagePath))
						err = templates.ExecuteTemplate(w, pagePath[len("templates/"):], suc)
						if err != nil {
						...
						var templates = template.Must(template.ParseFiles(pagePath))
						err = templates.ExecuteTemplate(w, pagePath[len("templates/"):], fail)
						if err != nil {
							http.Error(w, err.Error(), http.StatusInternalServerError)
						}
				...
				func gameHandler(w http.ResponseWriter, r * http.Request, arg string){
					...
					default:
						...
						var templates = template.Must(template.ParseFiles(pagePath))
						err = templates.ExecuteTemplate(w, pagePath[len("templates/"):], gn)
						if err != nil {
							http.Error(w, err.Error(), http.StatusInternalServerError)
						}
